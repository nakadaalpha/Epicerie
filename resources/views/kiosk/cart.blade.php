...
<head>
    ...
    <style>
        ...
        .custom-checkbox:checked::after {
            ...
            color: white; font-size: 0.75rem; position: absolute;
...
<body class="bg-gray-50 text-gray-700">
    ...
    <div class="max-w-[1150px] mx-auto px-4 py-8">
        ...
        <div class="flex flex-col items-center justify-center py-12 px-4">
            ...
            <p class="text-gray-500 mb-8 text-center max-w-md leading-relaxed">Belum ada barang nih. Yuk isi dengan kebutuhan harianmu!</p>
            ...
            <div class="w-full max-w-4xl border-t border-gray-200 pt-10 relative">
                <div class="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-50 px-4 text-gray-400 text-xs font-bold uppercase tracking-widest">Mungkin Anda Suka</div>
                    ...
                    @foreach($rekomendasi as $p)
                        ...
                        <div class="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between transition-all hover:shadow-md hover:border-blue-200 relative group overflow-hidden">
                            ...
                            <div class="flex justify-end mt-2 z-20 relative">
                                <a href="{{ route('kiosk.add', $p->id_produk) }}" class="bg-blue-600 hover:bg-blue-700 text-white w-8 h-8 flex items-center justify-center rounded-full shadow-md"><i class="fa-solid fa-plus"></i></a>
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <form action="{{ route('kiosk.checkout') }}" method="GET" id="checkoutForm">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-10">
                            <div class="flex items-center gap-3">
                                ...
                                <label for="check-all" class="font-bold text-gray-700 cursor-pointer select-none">Pilih Semua (<span id="total-items-header">{{ count($keranjang) }}</span>)</label>
                            </div>
                                ...
                                <div class="flex gap-4 w-full">
                                    <div class="flex gap-4 shrink-0 items-start">
                                        <div class="pt-2">
                                            <input type="checkbox" 
                                                   name="selected_ids[]" 
                                                   value="{{ $item->id_keranjang ?? $item->id }}" 
                                                   data-price="{{ $item->produk->harga_final * $item->jumlah }}"
                                    ...
                                    <div class="flex-1 flex flex-col sm:flex-row justify-between items-start gap-4">
                                        ...
                                        <div class="flex flex-row sm:flex-col items-end justify-between w-full sm:w-auto gap-4 sm:gap-6 mt-2 sm:mt-0">
                                            ...
                                            <div class="flex items-center gap-3 order-2 sm:order-2">
                                                <button type="button" onclick="openModal('{{ route('kiosk.remove', $item->id_produk) }}')" class="text-gray-400 hover:text-gray-600 transition p-1" title="Hapus Barang">
                ...
                <div class="lg:col-span-4">
                    <div class="bg-white p-5 rounded-xl shadow-[0_1px_6px_rgba(0,0,0,0.1)] border border-gray-100 sticky top-28">
                        ...
                        <div class="flex justify-between items-center mb-4 text-gray-600 text-sm">
                            <span>Total Harga (<span id="total-selected-count">{{ count($keranjang) }}</span> barang)</span>
                            <span class="font-bold text-gray-800" id="subtotal-display">Rp{{ number_format($subtotal, 0, ',', '.') }}</span>
                        </div>
    ...
    <script>
        ...
        // FUNGSI BARU: MENGHITUNG TOTAL SECARA REALTIME
        function calculateTotal() {
            ...
            checkboxes.forEach(cb => {
                // Ambil harga dari atribut data-price (pastikan konversi ke float/int)
