<!DOCTYPE html>
<html lang="id">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>{{ $produk->nama_produk }} - Detail Produk</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>

<body class="bg-gray-100">

  <div class="max-w-6xl mx-auto px-6 py-10">

    {{-- Tombol Kembali --}}
    <div class="mb-4">
      <a href="{{ route('kiosk.index') }}"
         class="inline-flex items-center gap-1 text-sm text-blue-600 hover:underline font-medium">
        ← Kembali ke daftar produk
      </a>
    </div>

    {{-- Breadcrumb --}}
    <div class="text-sm text-gray-500 mb-6">
      <a href="{{ route('kiosk.index') }}" class="hover:underline">Home</a>
      <span class="mx-2">›</span>
      <span class="text-gray-700 font-medium">
        {{ $produk->nama_produk }}
      </span>
    </div>

    @php
      $imgSrc = $produk->gambar
        ? asset('storage/' . ltrim($produk->gambar, '/'))
        : 'https://via.placeholder.com/600x600?text=Gambar+Produk';
    @endphp

    {{-- Card Detail Produk --}}
    <div class="bg-white rounded-xl shadow-sm p-8 grid grid-cols-1 md:grid-cols-2 gap-10">

      {{-- Gambar Produk --}}
      <div>
        <div class="border rounded-lg p-6 flex items-center justify-center">
          <img
            src="{{ $imgSrc }}"
            alt="{{ $produk->nama_produk }}"
            class="max-h-[420px] object-contain"
          >
        </div>
      </div>

      {{-- Informasi Produk --}}
      <div>
        <h1 class="text-2xl font-semibold text-gray-800 mb-2">
          {{ $produk->nama_produk }}
        </h1>

        <div class="text-2xl font-bold text-gray-900 mb-6">
          Rp {{ number_format($produk->harga_produk, 0, ',', '.') }}
        </div>

        {{-- Tombol Keranjang --}}
        <div class="mb-8">
          <a href="{{ route('kiosk.add', $produk->id_produk) }}"
             class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-full transition">
            <span class="text-xl">+</span> Keranjang
          </a>
        </div>

        <hr class="mb-6">

        {{-- Deskripsi --}}
        <div>
          <h2 class="font-semibold text-gray-800 mb-2">
            Deskripsi Produk
          </h2>
          <p class="text-gray-600 leading-relaxed">
            {{ $produk->deskripsi_produk ?? 'Belum ada deskripsi produk.' }}
          </p>
        </div>
      </div>
    </div>

    {{-- Rekomendasi Produk --}}
    @if(isset($rekomendasi) && count($rekomendasi) > 0)
    <div class="mt-12">
      <h2 class="text-lg font-bold text-gray-800 mb-4">
        Rekomendasi Produk
      </h2>

      <div x-data class="relative">

        {{-- Tombol Scroll Kiri --}}
        <button
          @click="$refs.slider.scrollLeft -= 300"
          class="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow px-2 py-2 rounded-full hover:bg-blue-100">
          <i class="fa-solid fa-chevron-left text-blue-600"></i>
        </button>

        {{-- Slider --}}
        <div
          x-ref="slider"
          class="overflow-x-auto no-scrollbar flex gap-4 px-8 pb-2">

          @foreach($rekomendasi as $item)
          <div class="min-w-[160px] bg-white rounded-xl shadow p-3 flex-shrink-0 hover:shadow-md transition">
            <a href="{{ route('produk.show', $item->id_produk) }}">
              <img
                src="{{ asset('storage/' . $item->gambar) }}"
                alt="{{ $item->nama_produk }}"
                class="w-full h-24 object-cover rounded-lg mb-2"
              >
              <h3 class="text-sm font-bold text-gray-800 truncate">
                {{ $item->nama_produk }}
              </h3>
              <p class="text-xs text-blue-600 font-semibold">
                Rp {{ number_format($item->harga_produk, 0, ',', '.') }}
              </p>
            </a>

            <a href="{{ route('kiosk.add', $item->id_produk) }}"
               class="mt-2 block text-center bg-orange-500 text-white text-xs font-bold px-3 py-2 rounded-lg shadow hover:bg-orange-600 transition">
              + Keranjang
            </a>
          </div>
          @endforeach
        </div>

        {{-- Tombol Scroll Kanan --}}
        <button
          @click="$refs.slider.scrollLeft += 300"
          class="absolute right-0 top-1/2 -translate-y-1/2 z-10 
                w-12 h-12 flex items-center justify-center 
                bg-white border border-gray-200 shadow-md 
                rounded-full hover:bg-blue-50 hover:shadow-lg 
                transition transform hover:scale-105">
          <i class="fa-solid fa-chevron-right text-blue-600 text-lg"></i>
        </button>

      </div>
    </div>
    @endif

  </div>

</body>
</html>
